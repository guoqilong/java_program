		...
		<form action="checkout" method="post">
			<table>
				...
				<c:forEach items="${cartlist }" var="item">
					<tr>
						<td class="ring-in"><a href="single.jsp" class="at-in"><img
								src="images/${item.pro.photo }" class="img-responsive" alt="" /></a>
							<div class="sed">
								<h5>${item.pro.name}</h5>
								<p>(${item.pro.info})</p>
						...
						<td class="check"><input type="text" value="${item.num}"
							onfocus="this.value='';"
							onblur="if(this.value == ''){this.value = ${item.num}}else{changeNum(${item.pro.id}, this.value, ${item.pro.price}, ${item.num})}"></td>
						<td class="checkk">$${item.pro.price}</td>
						<td>FREE SHIPPING</td>
						<td id="a${item.pro.id}">$${item.pro.price * item.num}</td>
						<!-- 批量删除 -->
						<td><input type="checkbox" name="test" value="${item.pro.id}"
							style="width: 17px; height: 17px; display: block; margin-left: 20px;" />
						</td>
					</tr>
				</c:forEach>
				<!--<tr>
		  ...
		  <td class="ring-in"><a href="single.jsp" class="at-in"><img src="images/ce2.jpg" class="img-responsive" alt=""></a>
			...
			<a href="generateorder" class=" to-buy">购买</a> <a
				href="javascript:void(0)" class=" to-buy"
				style="position: relative; left: 850px; top: -15px"
