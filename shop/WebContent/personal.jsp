...
<div id="box1">
	<!-- 应该是写到User里 -->
	<form>
		<span>更改密码</span> <input type="password" id="password" /><br />
		<br /> <input type="button" value="提交" onclick="changePassword()" />
		<br />
		<br />
		<br />
		<br /> <span>新增信息</span>
		<textarea rows="8" cols="20" id="info"></textarea>
		<br />
		<br /> <input type="button" value="更新" onclick="addInfo()" />
	</form>
</div>
...
<div id="box2">
	...
	<c:forEach items="${curorder}" var="item">
		<p>${item.pro.name}:$${item.pro.price} * ${item.num} =
			$${item.pro.price * item.num}</p>
	</c:forEach>
	<h4>Total : $${total}.0</h4>
</div>
...
<div id="box3">
	<center>
		<c:forEach items="${page.pages }" var="c">
			<a href="javascript:void(0)" onclick="changeOrder(${c })"
				class="pagee">${c }</a>
		</c:forEach>
	</center>
</div>
